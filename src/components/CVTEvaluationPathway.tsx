...
export default function CVTEvaluationPathway() {
  ...
  const openEtiology = () => {
    ...
    setAccordionValue((prev) => (prev.includes("etiology") ? prev : [...prev, "etiology"]));
  };
          ...
          <CardHeader className="bg-purple-100/50 dark:bg-purple-900/30">
            <CardTitle className="flex items-center justify-between text-purple-800 dark:text-purple-200">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-base sm:text-lg truncate">{CVT_PATHWAY_TITLE}</span>
                <Badge className="ml-1 bg-purple-600 text-white hidden sm:inline-flex">SNIF pathway</Badge>
              </div>
        ...
        <CollapsibleContent>
          <CardContent className="pt-5 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div
                className={cn(
                  "rounded-lg border-2 px-4 py-3 dark:bg-sky-950/20",
                  state.imaging !== "pending"
                    ? "border-sky-500 bg-sky-50 ring-2 ring-sky-300/60 dark:border-sky-400 dark:ring-sky-700/50"
                    : "border-sky-300 bg-sky-50 dark:border-sky-700",
                )}
              >
                ...
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <ChoiceButton
                    accent="slate"
                    selected={state.imaging === "no-cvt"}
                    onClick={() => setImaging(state.imaging === "no-cvt" ? "pending" : "no-cvt")}
                  >
                    No evidence of CVT
                  </ChoiceButton>
                  <ChoiceButton
              ...
              {noCvt && (
                <>
                  <FlowArrow />
                  <div className="rounded-lg border-2 border-slate-300 bg-slate-50 p-3 dark:border-slate-600 dark:bg-slate-900/40">
                    <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 mb-2">
                      Consider alternative diagnoses
                    </p>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-xs text-slate-700 dark:text-slate-300">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-stretch">
                    ...
                    >
                      ...
                      <p className="text-[11px] text-cyan-700 dark:text-cyan-300 mt-1">
                        Urgent neurosurgical review and etiological workup run in parallel with anticoagulation
                      </p>
                    </div>
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-purple-700 dark:text-purple-300">
                        Open the workup checklist — clinical, exposures, initial labs, and additional tests
                      </p>
                    </button>
                  </div>
            ...
            <Accordion type="multiple" value={accordionValue} onValueChange={setAccordionValue} className="w-full">
              <AccordionItem value="etiology" className="border-purple-200 dark:border-purple-800">
                <AccordionTrigger className="text-sm font-semibold text-purple-800 dark:text-purple-200 hover:no-underline">
                  Etiological evaluation — workup checklist
                </AccordionTrigger>
                <AccordionContent>
                  <div className="rounded-lg border border-purple-200 bg-purple-50/70 p-3 dark:border-purple-800 dark:bg-purple-950/20">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <EtiologyColumn
                        title={CVT_ETIOLOGY.additionalLabs.title}
                        items={CVT_ETIOLOGY.additionalLabs.items}
                      />
                    </div>
                  </div>
                </AccordionContent>
